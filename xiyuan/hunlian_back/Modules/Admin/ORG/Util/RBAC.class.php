...
class RBAC {
    ...
    static public function getAccessList($authId)
    {
        ...
        if(!$_SESSION["administrator"]){
        	$sql.=" user.user_id='{$authId}' and ";
        }
        $sql.="  user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=1 and node.status=1 group by node.id";
        $apps =   $db->query($sql);
        ...
        foreach($apps as $key=>$app) {
            ...
            $sql    =   "select node.id,node.name from ".
                    ...
                    $table['user']." as user,".
                    $table['access']." as access ,".
                    $table['node']." as node where ";
	        if(!$_SESSION["administrator"]){
	        	$sql.=" user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and  ";
	        }
	        $sql.=" access.node_id=node.id and node.level=2 and node.pid={$appId} and node.status=1 group by node.id";

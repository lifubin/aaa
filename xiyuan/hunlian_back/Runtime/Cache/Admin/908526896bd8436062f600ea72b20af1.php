...
<!--End-breadcrumbs-->
	<div class="widget-box">
		<div class="widget-content">
			<a href="__URL__/add" class="btn btn-info">新建</a>
		</div>
		<div class="widget-content">
	       <table class="table table-bordered table-striped">
	         ...
	         <tbody>
	        	<?php if(is_array($nodeList)): $i = 0; $__LIST__ = $nodeList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr class="odd gradeX">
		             <td><?php echo ($item["id"]); ?></td>
		             <td><?php echo str_repeat("&nbsp;",$item['level']*3); ?>|---<?php echo ($item["title"]); ?></td>
		             <td><?php echo ($item["name"]); ?></td>
		             <td class="center"><?php echo ($item["sort"]); ?></td>
		             <td class="center"><?php if($item["status"]){echo "可用";}else{echo "不可用"; } ?></td>
		             <td class="center"> <a href="__URL__/edit/id/<?php echo ($item['id']); ?>" class="btn btn-primary btn-mini">修改</a> <a href="#" class="btn btn-danger btn-mini">删除</a></td>

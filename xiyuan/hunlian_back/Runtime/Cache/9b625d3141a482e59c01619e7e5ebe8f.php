            ...
            <form class="form-horizontal" method="post" action="__URL__/update" name="role_form" id="role_form" novalidate="novalidate">
              ...
              <div class="control-group">
               ...
               <div class="control-group">
                ...
                <div class="controls">
                  <input type="text" name="ename" id="ename" value="<?php echo ($vo["ename"]); ?>"  >
                </div>
              </div>
              
              <div class="control-group">
              	<?php if(is_array($nodelist)): $i = 0; $__LIST__ = $nodelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><label class="control-label"><div class="checker" id="uniform-undefined"><span class=""><input type="checkbox" class="<?php echo ($item['name']); ?>" name="node[<?php echo ($item['id']); ?>]" value="<?php echo ($item['id']); ?>" style="opacity: 0;" <?php if(isset($accesslist[$item['id']])){echo "checked=checked";} ?> ></span></div>
	                  <?php echo ($item["title"]); ?></label>
	              <div class="controls">
	              	<?php if(is_array($item[subnode])): $i = 0; $__LIST__ = $item[subnode];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item1): $mod = ($i % 2 );++$i;?><label>
		                  <div class="checker" id="uniform-undefined"><span class=""><input type="checkbox" class="<?php echo ($item['name']); ?> <?php echo ($item1['name']); ?>" name="node[<?php echo ($item1['id']); ?>]" value="<?php echo ($item1['id']); ?>" style="opacity: 0;" <?php if(isset($accesslist[$item1['id']])){echo "checked=checked";} ?> ></span></div>
		                  <?php echo ($item1["title"]); ?>
		                </label>
		                <div style="margin:15px 15px 15px 20px;">
		                	<?php if(is_array($item1[subnode])): $i = 0; $__LIST__ = $item1[subnode];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item2): $mod = ($i % 2 );++$i;?><label>
				                  <div class="checker" id="uniform-undefined"><span class=""><input type="checkbox" class="<?php echo ($item['name']); ?> <?php echo ($item1['name']); ?> <?php echo ($item2['name']); ?>" value="<?php echo ($item2['id']); ?>" name="node[<?php echo ($item2['id']); ?>]" style="opacity: 0;"  <?php if(isset($accesslist[$item2['id']])){echo "checked=checked";} ?> ></span></div>
